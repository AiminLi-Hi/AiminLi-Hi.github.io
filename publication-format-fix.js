(() => {
  ...
  function removeRecentPublicationHeading() {
    document.querySelectorAll('#publications h3').forEach(heading => {
      const text = heading.textContent.trim().toLowerCase();
      if (text === 'recent publications' || text === 'all other publications') {
        heading.remove();
      }
    });
  }

  function normalizeDynamicVenueButtons() {
    const dynamicVenues = new Set(['IEEE TMC', 'IEEE IoTJ']);
    document.querySelectorAll('#publications button').forEach(button => {
      const venue = button.textContent.trim().replace(/\s*\(\d+\)\s*$/, '');
      if (!dynamicVenues.has(venue)) return;
      button.className = 'homepage-dynamic-venue-button';
  ...
  function apply() {
    addStyles();
    removeRecentPublicationHeading();
    normalizeDynamicVenueButtons();
    ...
    return document.querySelectorAll('[data-homepage-publication]').length > 0 ||
      document.querySelector('[data-homepage-venue-button]') !== null;
  }
